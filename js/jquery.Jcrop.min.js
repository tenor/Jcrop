...
"-holder").css({position:"relative",backgroundColor:b.bgColor}).insertAfter(K).append(r);b.overlayOffsetX=Aa-F.offset().left;b.overlayOffsetY=Ba-F.offset().top;delete b.bgColor;b.addClass&&F.addClass(b.addClass);var ga=p("<img />").attr("src",r.attr("src")).css(ya).width(z).height(v),na=p("<div />").width(H(100)).height(H(100)).css({zIndex:310,position:"absolute",overflow:"hidden"}).append(ga),V=p("<div />").width(H(100)).height(H(100)).css("zIndex",320),W=p("<div />").css({position:"absolute",zIndex:300}).insertBefore(r).append(na,
V);ma&&W.css({overflowY:"hidden"});var X=b.boundary,P=ta().width(z+X*2).height(v+X*2).css({position:"absolute",top:t(-X),left:t(-X),zIndex:290}).mousedown(ra),fa=b.bgOpacity,$,aa,ba,ca,C,E,U,Ca;Z=T(r);var Q=function(){function a(){var f={},g=["touchstart","touchmove","touchend"],n=document.createElement("div"),d;try{for(d=0;d<g.length;d++){var e=g[d];e="on"+e;var j=e in n;if(!j){n.setAttribute(e,"return;");j=typeof n[e]=="function"}f[g[d]]=j}return f.touchstart&&f.touchend&&f.touchmove}catch(i){return false}}
return{createDragger:function(f){return function(g){g.pageX=g.originalEvent.changedTouches[0].pageX;g.pageY=g.originalEvent.changedTouches[0].pageY;if(b.disabled)return false;if(f==="move"&&!b.allowMove)return false;U=true;O(f,N(g));g.stopPropagation();g.preventDefault();return false}},newSelection:function(f){f.pageX=f.originalEvent.changedTouches[0].pageX;f.pageY=f.originalEvent.changedTouches[0].pageY;return ra(f)},isSupported:a,support:b.touchSupport===true||b.touchSupport===false?b.touchSupport:
a()}}(),u=function(){function a(){if(!b.aspectRatio){var c=j-d,m=i-e;if($&&Math.abs(c)>$)j=c>0?d+$:d-$;if(aa&&Math.abs(m)>aa)i=m>0?e+aa:e-aa;if(ca/E&&Math.abs(m)<ca/E)i=m>0?e+ca/E:e-ca/E;if(ba/C&&Math.abs(c)<ba/C)j=c>0?d+ba/C:d-ba/C;if(d<0){j-=d;d-=d}if(e<0){i-=e;e-=e}if(j<0){d-=j;j-=j}if(i<0){e-=i;i-=i}if(j>z){c=j-z;d-=c;j-=c}if(i>v){c=i-v;e-=c;i-=c}if(d>z){c=d-v;i-=c;e-=c}if(e>v){c=e-v;i-=c;e-=c}return n(g(d,e,j,i))}c=b.aspectRatio;m=b.minSize[0]/C;var A=b.maxSize[0]/C,q=j-d,D=i-e,o=Math.abs(q),
l=Math.abs(D),k=o/l;if(A===0)A=z*10;if(k<c){o=i;w=l*c;l=q<0?d-w:w+d;if(l<0){l=0;h=Math.abs((l-d)/c);o=D<0?e-h:h+e}else if(l>z){l=z;h=Math.abs((l-d)/c);o=D<0?e-h:h+e}}else{l=j;h=o/c;o=D<0?e-h:e+h;if(o<0){o=0;w=Math.abs((o-e)*c);l=q<0?d-w:w+d}else if(o>v){o=v;w=Math.abs(o-e)*c;l=q<0?d-w:w+d}}if(l>d){if(l-d<m)l=d+m;else if(l-d>A)l=d+A;o=o>e?e+(l-d)/c:e-(l-d)/c}else if(l<d){if(d-l<m)l=d-m;else if(d-l>A)l=d-A;o=o>e?e+(d-l)/c:e-(d-l)/c}if(l<0){d-=l;l=0}else if(l>z){d-=l-z;l=z}if(o<0){e-=o;o=0}else if(o>
v){e-=o-v;o=v}return n(g(d,e,l,o))}function f(c){if(c[0]<0)c[0]=0;if(c[1]<0)c[1]=0;if(c[0]>z)c[0]=z;if(c[1]>v)c[1]=v;return[c[0],c[1]]}function g(c,m,A,q){var D=c,o=A,l=m,k=q;if(A<c){D=A;o=c}if(q<m){l=q;k=m}return[Math.round(D),Math.round(l),Math.round(o),Math.round(k)]}function n(c){return{x:c[0],y:c[1],x2:c[2],y2:c[3],w:c[2]-c[0],h:c[3]-c[1]}}var d=0,e=0,j=0,i=0,L,M;return{flipCoords:g,setPressed:function(c){c=f(c);j=d=c[0];i=e=c[1]},setCurrent:function(c){c=f(c);L=c[0]-j;M=c[1]-i;j=c[0];i=c[1]},
getOffset:function(){return[L,M]},moveOffset:function(c){var m=c[0];c=c[1];if(0>d+m)m-=m+d;if(0>e+c)c-=c+e;if(v<i+c)c+=v-(i+c);if(z<j+m)m+=z-(j+m);d+=m;j+=m;e+=c;i+=c},getCorner:function(c){var m=a();switch(c){case "ne":return[m.x2,m.y];case "nw":return[m.x,m.y];case "se":return[m.x2,m.y2];case "sw":return[m.x,m.y2]}},getFixed:a}}(),s=function(){function a(k){k=p("<div />").css({position:"absolute",opacity:b.borderOpacity}).addClass(b.baseClass+"-"+k);na.append(k);return k}function f(k,B){var G=p("<div />").mousedown(pa(k)).css({cursor:k+
"-resize",position:"absolute",zIndex:B});Q.support&&G.bind("touchstart",Q.createDragger(k));V.append(G);return G}function g(k){var B=b.handleSize,G=B;B=B;var Y=o,Da=o;switch(k){case "n":case "s":B=H(100);break;case "e":case "w":G=H(100)}return f(k,A++).width(B).height(G).css({top:t(-Y+1),left:t(-Da+1)})}function n(k){var B;for(B=0;B<k.length;B++)q[k[B]]=f(k[B],A++).css({top:t(-o+1),left:t(-o+1),opacity:b.handleOpacity}).addClass(b.baseClass+"-handle")}function d(k){var B=Math.round(k.h/2-o),G=Math.round(k.w/
...
b.overlayOffsetY),left:t(B+b.overlayOffsetX)});D&&d(k);if(!m){W.show();b.bgFade?r.fadeTo(b.fadeTime,fa):r.css("opacity",fa);m=true}b.onChange.call(R,ka(k))}function L(){D=true;if(b.allowResize){d(u.getFixed());V.show();return true}}function M(){D=false;V.hide()}function c(k){void 0===k?M():L()}var m,A=370,q={},D=false,o=b.handleOffset;if(b.drawBorders){a("hline");a("hline bottom");a("vline");a("vline right")}if(b.dragEdges){q.t=g("n");q.b=g("s");q.r=g("e");q.l=g("w")}b.sideHandles&&n(["n","s","e",
"w"]);b.cornerHandles&&n(["sw","nw","ne","se"]);var l=ta().mousedown(pa("move")).css({cursor:"move",position:"absolute",zIndex:360});Q.support&&l.bind("touchstart.jcrop",Q.createDragger("move"));na.append(l);M();return{updateVisible:j,update:i,release:function(){M();W.hide();b.bgFade?r.fadeTo(b.fadeTime,1):r.css("opacity",1);m=false;b.onRelease.call(R)},refresh:e,isAwake:function(){return m},setCursor:function(k){l.css("cursor",k)},enableHandles:L,enableOnly:function(){D=true},showHandles:function(){if(D){d(u.getFixed());
...
drawBorders:true,dragEdges:true,fixedSupport:true,touchSupport:null,boxWidth:0,boxHeight:0,boundary:2,fadeTime:400,animationDelay:20,swingSpeed:3,minSelect:[0,0],maxSize:[0,0],minSize:[0,0],minSize:[0,0],overlayOffsetX:0,overlayOffsetY:0,onChange:function(){},onSelect:function(){},onRelease:function(){}}})(jQuery);
